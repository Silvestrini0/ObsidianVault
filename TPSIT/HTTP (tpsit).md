...
### Generalità
- **QUIC** è un protocollo di trasporto sviluppato da Google, standardizzato nella **RFC 9000**.
- Opera sopra **UDP**, ma implementa internamente affidabilità, controllo della congestione e multiplexing, superando alcuni limiti di TCP.
### Affidabilità e controllo della congestione
- **Numeri di pacchetto** univoci: consentono di rilevare perdite, identificare duplicati e ricostruire l’ordine corretto.
- **Ack frame**: le conferme sono trasportate come frame nel payload dei pacchetti QUIC; il mittente ritrasmette se non riceve l’Ack.
- **Controllo della congestione**: algoritmi interni (simili a quelli TCP) regolano dinamicamente la velocità di invio in base alle condizioni di rete.
### Multiplexing e gestione indipendente degli stream
- I dati sono suddivisi in **frame**, ciascuno associato a uno **stream indipendente** (identificato da uno stream ID).
- Ogni stream mantiene una propria sequenza di dati, controllo di flusso, affidabilità e ritrasmissioni.
- I frame di stream diversi vengono interlacciati all’interno degli stessi pacchetti QUIC, permettendo comunicazione parallela senza blocco.
- In caso di perdita di un pacchetto, vengono ritrasmessi solo i dati degli stream coinvolti, senza impattare gli altri.
### Sicurezza
- QUIC integra **TLS 1.3** direttamente, garantendo crittografia e autenticazione end‑to‑end sin dall’inizio della connessione.
### Incapsulamento
- I frame sono inseriti in pacchetti QUIC, che viaggiano su UDP. All’interno dello stesso pacchetto possono coesistere frame di stream diversi.
